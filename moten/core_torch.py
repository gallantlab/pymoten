...
def raw_project_stimulus(stimulus,
    ...
    Parameters
    ----------
    stimulus : torch.Tensor, (nimages, vdim, hdim) or (nimages, npixels)
        The movie frames.
    ...
    sin_responses = torch.zeros((nimages, nfilters), dtype=dtype, device=device)
    cos_responses = torch.zeros((nimages, nfilters), dtype=dtype, device=device)

    for gaborid, gabor_parameters in iterator_func(enumerate(filters),
                                                   'project_stimulus',
                                                   total=len(filters)):

        gabor_components = mk_3d_gabor(vhsize, **gabor_parameters)

        gabor_components = map(torch.from_numpy, gabor_components)
        gabor_components = map(partial(torch.Tensor.type, dtype=dtype), gabor_components)
        gabor_components = map(partial(torch.Tensor.to, device=device), gabor_components)
 	sgabor0, sgabor90, tgabor0, tgabor90 = gabor_components
...
def project_stimulus(stimulus,
    ...
    nfilters = len(filters)
    nimages = stimulus.shape[0]
    filter_responses = torch.zeros((nimages, nfilters), dtype=dtype, device=device)
    for gaborid, gabor_parameters in iterator_func(enumerate(filters),
                                                   'project_stimulus',
                                                   total=len(filters)):
...
def dotspatial_frames(spatial_gabor_sin, spatial_gabor_cos,
    ...
    Parameters
    ----------
    spatial_gabor_sin, spatial_gabor_cos : torch.Tensor, (vdim,hdim)
    ...
    if isinstance(mask, Number):
        ...
        assert mask.dtype in (torch.uint8, torch.bool)
...
def dotdelay_frames(spatial_gabor_sin, spatial_gabor_cos,
    ...
    gabor_sin, gabor_cos = dotspatial_frames(spatial_gabor_sin, spatial_gabor_cos,
                                             stimulus, mask=mask)
    gabor_prod = torch.stack([gabor_sin, gabor_cos], axis=1)
...
def compute_spatial_gabor_responses(stimulus,
    ...
    for idx, gabor_param_dict in iterator_func(enumerate(filters),
        ...
        gabor_components = mk_3d_gabor(vhsize, **gabor_param_dict)

        gabor_components = map(torch.from_numpy, gabor_components)
        gabor_components = map(partial(torch.Tensor.type, dtype=dtype), gabor_components)
        gabor_components = map(partial(torch.Tensor.to, device=device), gabor_components)

        sgabor_sin, sgabor_cos, _, _ = gabor_components
...
def compute_filter_responses(stimulus,
    ...
    for idx, gabor_param_dict in iterator_func(enumerate(filters),
        ...
        gabor0, gabor90, tgabor0, tgabor90 = gabor_components
    ...
    channels = output_nonlinearity(channels)
    if dozscore:
        channels -= channels.mean(0)
        channels /= channels.std(0) + 1e-19
    return channels
