...
3. All core computational functions work with both backends
"""
import numpy as np
import pytest
...
class TestBackendMechanism:
    def setup_method(self):
        """Reset to numpy backend before each test."""
        set_backend("numpy")
    ...
    def test_set_backend_warn_on_error(self):
        # Should warn and fall back, not raise
...
# Precision tolerance for float64 comparison between numpy and torch.
# Torch and numpy use different implementations of trig/exp functions
# and different operation ordering, leading to ~1e-6 differences in float64.
# These tolerances are reasonable for scientific computing reproducibility.
ATOL_F64 = 1e-5
RTOL_F64 = 1e-5
...
def _get_tolerances(backend_name):
    """Return (atol, rtol) for a given backend."""
...
def _get_tolerances_f32(backend_name):
    """Return (atol, rtol) for float32 comparisons."""
    if backend_name == "torch_mps":
        return ATOL_MPS, RTOL_MPS
    return ATOL_F32, RTOL_F32


# Small reproducible stimulus for testing
...
# Test parameters for a small pyramid
...
# Parametrize over non-numpy backends that are available
...
class TestMk3dGaborEquivalence:
    ...
    @pytest.mark.parametrize("backend_name", _NON_NUMPY_BACKENDS)
